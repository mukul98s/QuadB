import React from "react";
import { format } from "../helper/format";
import { VscTriangleDown, VscTriangleUp } from "react-icons/vsc";
const platforms = [
  {
    id: 1,
    name: "WazirX",
    ltp: "2645719",
    buy: "2642998",
    sell: "2642996",
    difference: "0.26 ",
    saving: "6924",
    logo: "data:image/png;base64,",
    pos: true,
  },
  {
    id: 2,
    name: "Bitbns",
    ltp: "2652967 ",
    buy: "2648358",
    sell: "2652967",
    difference: "0.65 ",
    saving: "17014",
    logo: "https://hodlinfo.com/static/media/bitbns.dcee6cf4.png",
    pos: true,
  },
  {
    id: 3,
    name: "Giotus",
    ltp: "2646000",
    buy: "2644500",
    sell: "2693000",
    difference: "-0.08 ",
    saving: "2052",
    logo: "data:image/png;base64,",
    pos: false,
  },
  {
    id: 4,
    name: "Colodax",
    ltp: "2583138",
    buy: "2675167",
    sell: "2674570",
    difference: "0.54 ",
    saving: "14352",
    logo: "data:image/png;base64,",
    pos: true,
  },
  {
    id: 5,
    name: "Zebpay",
    ltp: "2653990",
    buy: "2655990",
    sell: "2652000",
    difference: "-0.17 ",
    saving: "4497",
    logo: "https://hodlinfo.com/static/media/zebpay.232215c8.png",
    pos: false,
  },
  {
    id: 6,
    name: "CoinDCX",
    ltp: "2639910",
    buy: "2640759",
    sell: "2651803",
    difference: "-0.42 ",
    saving: "11187",
    logo: "data:image/png;base64,",
    pos: false,
  },
];

const Platform = () => {
  return (
    <section>
      <div className="platform">
        <table>
          <thead>
            <tr>
              <th>
                <h4>#</h4>
              </th>
              <th>
                <h4>Platform</h4>
              </th>
              <th>
                <h4>Last Traded Price</h4>
              </th>
              <th>
                <h4>Buy / Sell Price</h4>
              </th>
              <th>
                <h4>Difference</h4>
              </th>
              <th>
                <h4>Saving</h4>
              </th>
            </tr>
          </thead>
          <tbody>
            {platforms.map((platform) => {
              const {
                id,
                name,
                ltp,
                buy,
                sell,
                difference,
                saving,
                logo,
                pos,
              } = platform;
              return (
                <tr key={id}>
                  <td>
                    <h4>{id}</h4>
                  </td>
                  <td>
                    <a href="?">
                      <img src={logo} className="exchange-logo" alt={name} />
                      <h4>{name}</h4>
                    </a>
                  </td>
                  <td>
                    <h4>{format(ltp)}</h4>
                  </td>
                  <td>
                    <h4>
                      {format(buy)} / {format(sell)}
                    </h4>
                  </td>
                  <td>
                    <h4
                      className={`${
                        parseFloat(difference) > 0 ? "positive" : "danger"
                      }`}
                    >
                      {difference} %
                    </h4>
                  </td>
                  <td>
                    <h4 className={`${pos ? "positive" : "danger"}`}>
                      {pos ? <VscTriangleUp /> : <VscTriangleDown />}
                      {format(saving)}
                    </h4>
                  </td>
                </tr>
              );
            })}
          </tbody>
        </table>
      </div>
      <div className="platform__banner">
        <img
          src="https://hodlinfo.com/static/media/BannerFTXNews.d0cd974b.png"
          alt="platform"
        />
      </div>
    </section>
  );
};

export default Platform;
